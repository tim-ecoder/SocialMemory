#!/usr/bin/env python3
"""Tests for gold_silver_png_generator.

Verifies geometry, that only the two intended colors appear, mono-color
blocks, the gold:silver frequency ratio, determinism via seed, and that
...
import gold_silver_png_generator as gen
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for br in range(gen.ROWS):
            for bc in range(gen.COLS):
                ...
                assert color in (gen.GOLD, gen.SILVER), f"unexpected color {color}"
                seen.add(color)

        assert seen == {gen.GOLD, gen.SILVER}, f"both colors should appear, got {seen}"
        print("test_basic_properties: OK")


def test_gold_silver_ratio():
    # Count blocks across several seeds; gold should be ~GOLD_RATIO x silver.
    gold = silver = 0
    for seed in range(8):
        with tempfile.TemporaryDirectory() as d:
            path = os.path.join(d, "out.png")
            ...
            for br in range(gen.ROWS):
                for bc in range(gen.COLS):
                    ...
                    if c == gen.GOLD:
                        ...
                        silver += 1
    ratio = gold / silver
    expected = gen.GOLD_RATIO
    assert expected * 0.9 <= ratio <= expected * 1.1, \
        f"gold:silver ratio {ratio:.3f} not ~{expected}"
    print(f"test_gold_silver_ratio: OK (ratio={ratio:.3f})")
