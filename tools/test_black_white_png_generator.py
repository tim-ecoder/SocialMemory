#!/usr/bin/env python3
"""Tests for black_white_png_generator.

Verifies geometry, that only the two intended colors appear, mono-color
blocks, the black:white frequency ratio, determinism via seed, and that
...
import black_white_png_generator as gen
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for br in range(gen.ROWS):
            for bc in range(gen.COLS):
                ...
                assert color in (gen.BLACK, gen.WHITE), f"unexpected color {color}"
                seen.add(color)

        assert seen == {gen.BLACK, gen.WHITE}, f"both colors should appear, got {seen}"
        print("test_basic_properties: OK")


def test_black_white_ratio():
    # Count blocks across several seeds; black should be ~BLACK_RATIO x white.
    black = white = 0
    for seed in range(8):
        with tempfile.TemporaryDirectory() as d:
            path = os.path.join(d, "out.png")
            ...
            for br in range(gen.ROWS):
                for bc in range(gen.COLS):
                    ...
                    if c == gen.BLACK:
                        ...
                        white += 1
    ratio = black / white
    expected = gen.BLACK_RATIO
    assert expected * 0.9 <= ratio <= expected * 1.1, \
        f"black:white ratio {ratio:.3f} not ~{expected}"
    print(f"test_black_white_ratio: OK (ratio={ratio:.3f})")
