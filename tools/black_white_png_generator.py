...
Each "big pixel" is a 3x3 block painted one of two colors: black or white.
Black and white appear equally often by default. No third-party deps —
writes PNG by hand using only the standard library (zlib + struct).
"""
...
def make_pixel_grid(rng):
    ...
    Black blocks occur ~BLACK_RATIO x as often as white blocks (P_BLACK).
    """
    return [[BLACK if rng.random() < P_BLACK else WHITE
...
def main():
    p = argparse.ArgumentParser(description="Generate randomized black/white 3x3-pixel PNGs.")
    p.add_argument("-n", "--count", type=int, default=1, help="number of PNGs to generate")
    p.add_argument("-o", "--output", default="blackwhite.png",
